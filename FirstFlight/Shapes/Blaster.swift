...
final class Blaster: SKNode {
    ...
    func spawnBeamDebris(in scene: SKScene, count: Int) {
        guard isFiring, !beam.isHidden else { return }
        guard let beamTip = beamTipPosition(in: scene) else { return }

        for _ in 0..<count {
            Self.spawnDebrisParticle(in: scene, at: beamTip)
        }
    }

    private func beamTipPosition(in scene: SKScene) -> CGPoint? {
        let tipInBeam = CGPoint(x: 0, y: -beam.size.height)
        return beam.convert(tipInBeam, to: scene)
    }

    private static func spawnDebrisParticle(in scene: SKScene, at position: CGPoint) {
